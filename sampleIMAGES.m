...
for i=1:numpatches
    ...
    patchy = sample_images_noise(r:r+20,c:c+20,d);
    patches(:,i) = reshape(patch,[441 1]);
    patches_rain(:,i) = reshape(patchy,[441 1]);
    i
end
...
% means1 = mean(patches(:));
% pstd1 = 3*std(patches(:));
% means2 = mean(patches_rain(:));
% pstd2 = 3*std(patches_rain(:));
% patches = normalizeData(patches,means1,pstd1);
% patches_rain = normalizeData(patches_rain,means2,pstd2);
